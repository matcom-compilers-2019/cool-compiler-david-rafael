...
namespace cool.types
{
	public class ProgramSemantic  // a Program node in the AST
	{
		Dictionary<string, Type> types;
		List<Error> errors;
		List<string> basicTypes;
        #region Inicialize All Basicclass and metods
        ...
        #region Brute_ForceCycleChecking
	...
	public class Type : Statement //i.e a class
	{
		...
		{
            if (scope == null || scope.EnclosingType == null || (this.Name != scope.EnclosingType.Name && !isparent(this, scope.EnclosingType)))
			...
			foreach (Method m in methods)
			{
                ...
                if (parent != null)
				 baseMethod = Parent.Methods.Find(e => e.Name == m.Name); // At least parent will always be Object
				if (baseMethod != null)
				{
		...
		//	attributes.Sort(new AttributeComparer());
            ...
            {
                if (a.Id.Type.CheckSemantics(scope))
            ...
            foreach (Attribute a in Attributes)
			{
				...
				typeIsCorrect = typeIsCorrect && a.CheckSemantics(scope);// WFT??
			}
			return typeIsCorrect;
		}

        private bool isparent(Type type, Type enclosingType)
        {
            bool result = false;
            if (type.children.Contains(enclosingType)) return true;
            else
                foreach (var item in type.children)
                {
                   result= isparent(item, enclosingType) || result ;
                    if (result) break;
                }
            return result;
        }
	...
	public class Attribute : Expression
	{
		...
		*/
		...
		public override bool CheckSemantics(Scope scope)
		{
			...
			if (id.Exp != null)
			{
               // if (id.Type.Name == "Bool") if((id.Exp.Evaluate(scope).ToString() == "true") || (id.Exp.Evaluate(scope).ToString() == "false")) return true;
              ...
              //  if(id.Type.Name=="Bool"&& id.Exp.Type==null )
		...
		public override bool CheckSemantics(Scope scope)
		{
		  ...
		  of the object computed at runtime, which is potentially any subclass of A.) Class A must have a method
			...
			*/
            ...
            if (m == null) m = scope.EnclosingType.Methods.Find(e => e.Name == id);
...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    #region ErrorClass and abstrac position Cordenate
    public class Coord
	{
		int row, col;
    ...
    #region Compare from icompare<T> Method Attribute
