...
namespace cool.types
{
	 ...
	 */
	class VisitASTFactory : CoolBaseVisitor<object>
	{
        ProgramSemantic p;
		...
		public override object VisitAttribFeature([NotNull] CoolParser.AttribFeatureContext context)
		{
            ...
            {
                //Console.WriteLine(context.expresion());
		...
		{
           // if (context.id.Text == "true" || context.id.Text == "false") return VisitBoolExp((CoolParser.BoolExpContext) context);
		...
		public override object VisitIsvoidExp([NotNull] CoolParser.IsvoidExpContext context)
		{
			...
			return new Isvoid( ((cool.types.Expression)VisitChildren(context)), GetCoord(context));
		}
        ...
        public override object VisitBiggerEqualExp([NotNull] CoolParser.BiggerEqualExpContext context)
        {
            Expression left = (Expression)Visit(context.expresion()[0]);
            Expression right = (Expression)Visit(context.expresion()[1]);
    ...
    {
        ProgramSemantic p;
        public Type_check(ProgramSemantic p)
        {
            this.p = p;
        }
