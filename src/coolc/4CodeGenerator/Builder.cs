...
namespace CodeGenerator
{
    class Builder : CoolBaseVisitor<Node>
    {
        ...
        public void Compile(IParseTree tree,string exit)
        {
        ...
        private void chec_typeAdded(Node s, CoolParser.MethodFeatureContext context, string classname)
        {
            foreach (var son in s.Childs)
            {
                ...
                if (SymbolTable.Classes[classname].Attributes.ContainsKey(son.ToString()))
                {
                 ...
                 if (son.Childs.Count() > 0) foreach (var item in son.Childs)
                        {
        ...
        public override Node VisitBiggerEqualExp([NotNull] CoolParser.BiggerEqualExpContext context)
        {
            var s = VisitChildren(context);
            return new BiggerEqualNode(context, s);
        }
